...
from typing import ClassVar, Dict, List, NoReturn, Set
...
from ppln.utils.dist import all_gather_cpu
...
class EpochMetricHook(BaseHook):
    ...
    def __init__(
        self, handle: Dict[str, str], inputs_dim: int = 0, targets_dim: int = 0, outputs_key: str = "epoch_metric"
    ) -> NoReturn:
        self._handle = handle
    ...
    def after_iter(self, runner: Runner) -> NoReturn:
        predictions: Dict[str, Dict[str, torch.Tensor]] = runner.batch_outputs.get(self._outputs_key, None)
    ...
    def after_epoch(self, runner: Runner) -> NoReturn:
        for metric_name in self._handle.keys():
            ...
            if accumulated:
                ...
                runner.epoch_outputs[metric_name] = metric.item()
